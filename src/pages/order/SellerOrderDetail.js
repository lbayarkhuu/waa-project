                ...
                <div className="bg-white pb-2 shadow-sm rounded-sm">
                    <div className="text-gray-700">
                        <div className="grid md:grid-cols-2 text-sm">
                            ...
                            <div className="grid grid-cols-2">
                                <div className="px-4 py-2 font-semibold">Buyer</div>
                                <label className="px-4 py-2">{order.user.username}</label>
                            </div>
                            <div className="grid grid-cols-2">
            ...
            <div className="container px-5 mx-auto">
                <div className="bg-white p-3 shadow-sm rounded-sm">
                    <div className="flex items-center justify-center mt-4">
                        {order.status === "ACTIVE" &&
                            <button
                                className="px-4 ml-8 py-2 leading-5 text-white transition-colors duration-200 transform bg-gray-700 rounded hover:bg-gray-600 focus:outline-none"
                                type="button" onClick={()=>{changeStatus("SHIPPING")}}>Ship
                            </button>
                        }
                        {order.status === "SHIPPING" &&
                            <button
                                className="px-4 ml-8 py-2 leading-5 text-white transition-colors duration-200 transform bg-gray-700 rounded hover:bg-gray-600 focus:outline-none"
                                type="button" onClick={()=>{changeStatus("DELIVERED")}}>Delivered
                            </button>
                        }
                        {(order.status === "ACTIVE") &&
                            <button
